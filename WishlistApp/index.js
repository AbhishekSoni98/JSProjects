...
function renderTodoList() {
    ...
    todoList.forEach(({id, todo, isCompleted}) => {
        iH +=  `
            <li class="list-group-item ${isCompleted ? "bg-success" : "bg-danger"} mt-2" data-key=${id}>
                <input class="form-check-input mt-2" id="item-${id}" type="hidden" data-key=${id}>
                    <label for="item-${id}" class="todo todo-text t-pointer ${isCompleted ? "checked-todo" : ""}" data-key=${id}> &nbsp; ${todo}</label>
                    <button type="button" class="btn ${isCompleted ? "btn-success" : "btn-danger"}" data-todokey=${id}>
